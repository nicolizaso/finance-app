...
const BalanceCard = ({ transactions, isPrivacyMode }) => {
  ...
  return (
    <div className="bento-card h-full flex flex-col justify-between group">
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 to-transparent opacity-50 group-hover:opacity-100 transition-opacity duration-500"></div>
      
      <div className="relative z-10">
        <h2 className="text-slate-400 text-sm font-medium uppercase tracking-wider mb-1">Saldo Total</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mt-6 relative z-10">
        <div className="bg-slate-700/50 p-3 rounded-2xl border border-white/5 backdrop-blur-sm">
          <div className="flex items-center gap-2 mb-1">
            <div className="w-2 h-2 rounded-full bg-teal-400 shadow-[0_0_8px_rgba(45,212,191,0.5)]"></div>
            <span className="text-xs text-slate-400 font-bold">Ingresos</span>
          </div>
          <p className={`text-lg font-bold text-teal-400 ${isPrivacyMode ? 'blur-sm' : ''}`}>
        ...
        <div className="bg-slate-700/50 p-3 rounded-2xl border border-white/5 backdrop-blur-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400 font-bold">Gastos</span>
          </div>
